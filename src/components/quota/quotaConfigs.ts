...
const buildCodexQuotaWindows = (payload: CodexUsagePayload, t: TFunction): CodexQuotaWindow[] => {
  ...
  const FULLY_RECOVERED_TOLERANCE_SECONDS = 60;
  const WINDOW_META = {
  ...
  const isResetFullyRecovered = (
    ...
    return Math.abs(resetDistanceSeconds - windowSeconds) <= FULLY_RECOVERED_TOLERANCE_SECONDS;
  };
  ...
  const addWindow = (
    ...
    windows.push({
      ...
      usedPercent,
      resetLabel,
      isResetFullyRecovered: isResetFullyRecovered(window, fullyRecoveredWindowSeconds),
    });
  };
  ...
  nodes.push(
    ...windows.map((window) => {
      ...
      const windowLabel = window.labelKey
        ...
        : window.label;
      const resetClassName = window.isResetFullyRecovered
